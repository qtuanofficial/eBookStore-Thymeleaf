...
- Tên bảng: snake_case, số nhiều, tiếng Anh (users, books, order_details, ...).
- Tên cột: snake_case, tiếng Anh, rõ nghĩa (first_name, created_at, ...).
- Mỗi bảng phải có khóa chính (id, kiểu INT/AUTO_INCREMENT hoặc UUID).
- Các trường thời gian: dùng created_at, updated_at (kiểu DATETIME hoặc TIMESTAMP).
- Trường trạng thái: dùng ENUM hoặc VARCHAR, không dùng số magic.
- Đặt index cho các cột truy vấn nhiều (email, username, foreign key...).
- Đảm bảo khóa ngoại (FOREIGN KEY) cho các quan hệ.
- Không lưu dữ liệu lặp lại, tuân thủ chuẩn hóa 3NF.
- Không dùng kiểu dữ liệu không rõ ràng (tránh TEXT cho dữ liệu ngắn, tránh VARCHAR(255) cho mọi trường).
- Nếu có trường boolean, dùng tinyint(1) hoặc boolean.

### 13.2. Ý nghĩa và chuẩn từng bảng chính

#### users
- Lưu thông tin người dùng.
- Cột chính: id, first_name, last_name, username, email, password, phone_number, birth_of_date, avatar, created_at, updated_at, status.
- email, username phải unique.
- Không lưu plain password.

#### addresses
- Lưu địa chỉ giao hàng của user.
- Cột chính: id, user_id (FK), address_line, ward, district, city, country, postal_code, created_at.
- Một user có thể có nhiều địa chỉ.

#### books
- Lưu thông tin sách.
- Cột chính: id, title, author, price, cover, sub_category_id (FK), created_at, updated_at, status.
- Có thể có các trường bổ sung: is_premium, is_best_seller.

#### book_details
- Lưu chi tiết sách (1-1 với books).
- Cột chính: id, book_id (FK), isbn, publisher, publication_date, pages, description, summary.

#### categories
- Lưu danh mục cha.
- Cột chính: id, name, image, created_at.

#### sub_categories
- Lưu danh mục con.
- Cột chính: id, name, category_id (FK), created_at.

#### wishlists
- Lưu sách yêu thích của user.
- Cột chính: id, user_id (FK), book_id (FK), created_at.
- Một user không được wishlist trùng sách.

#### carts
- Lưu giỏ hàng của user.
- Cột chính: id, user_id (FK), created_at, updated_at.

#### cart_items
- Lưu từng sản phẩm trong giỏ hàng.
- Cột chính: id, cart_id (FK), book_id (FK), quantity, price, created_at.

#### orders (order_details)
- Lưu đơn hàng.
- Cột chính: id, user_id (FK), order_address, total, status, created_at, updated_at, note.
- Trạng thái: PENDING, CONFIRMED, PREPARING, SHIPPING, DELIVERED, CANCELLED.

#### order_items
- Lưu từng sản phẩm trong đơn hàng.
- Cột chính: id, order_id (FK), book_id (FK), quantity, price.

#### payments (payment_details)
- Lưu thông tin thanh toán cho đơn hàng.
- Cột chính: id, order_id (FK), provider, status, amount, created_at, updated_at.
- provider: COD, VNPAY, MOMO, VISA, BANKING.

#### reviews
- Lưu đánh giá sách của user.
- Cột chính: id, user_id (FK), book_id (FK), rating, comment, created_at, updated_at.
- Một user chỉ review 1 lần cho 1 sách.

### 13.3. Chuẩn hóa quan hệ

- 1 user - n addresses
- 1 user - n wishlists
- 1 user - n carts (thường chỉ 1 cart active)
- 1 cart - n cart_items
- 1 user - n orders
...
### 13.4. Lưu ý khi migration/seed

- Không xóa dữ liệu thật khi migrate.
- Seed data chỉ dùng cho môi trường dev/test.
- Đảm bảo dữ liệu mẫu không trùng với dữ liệu thật.
- Backup dữ liệu trước khi chạy migration lớn.
